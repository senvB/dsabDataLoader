...
final class LoaderUtil {
    ...
    static Elements loadHtmlSource(String url, String pattern) throws IOException {
        return Jsoup.connect(url).header(LoaderUtil.USER_AGENT_HEADER_KEY, LoaderUtil.USER_AGENT_HEADER_VALUE)
                .header("connection", "Keep-Alive").timeout(20000).get().select(pattern);
    }
    ...
    static boolean lineStartsWithANumber(String currentLine) {
        try {
            Integer.parseInt(currentLine.split(StringUtils.SPACE)[0]);
            return true;
    ...
    static boolean lineStartsWithANumberLessThan(String currentLine, int lessThan) {
        try {
            return Integer.parseInt(currentLine.split(StringUtils.SPACE)[0]) < lessThan;
        } catch (NumberFormatException e) {
            return false;
        }
    }
